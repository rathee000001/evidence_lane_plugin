...
This original source owner is `authorities/project_sectors/ppt`. Live project state belongs to `ppt` under the selected external project root. The packaged SQLite is an empty, unbound schema template; the engine creates identities and applies exact migrations under its writer. It is never opened as project state.
