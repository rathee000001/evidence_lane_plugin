"""artifacts: typed binding to the authenticated shared SDK and owning engine."""
from evidence_lane_plugin.sector_support import build_sector_source

LANE_ID = 'artifacts'
ACTION_LANES = {'artifacts_index': 'artifacts', 'artifacts_index_media': 'artifacts', 'artifacts_refresh': 'artifacts', 'artifacts_refresh_media': 'artifacts', 'source_snapshot_retire': 'artifacts'}

def build_lane_sources(client, **arguments):
    return build_sector_source(client, LANE_ID, ACTION_LANES, **arguments)

__all__ = ["LANE_ID", "build_lane_sources"]
