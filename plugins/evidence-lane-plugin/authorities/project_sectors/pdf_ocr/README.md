...
`schema.sql`, `migration-history.json` and `schema-template.json` describe the actual owner migrations. `pdf_ocr.mmd` and `pdf_ocr.dot` are matching schema traversal maps. `workflow.v4.json`, `workflow.mmd` and `workflow.dot` describe current action routes and their gates. They contain no project facts or runtime readiness assertion.
