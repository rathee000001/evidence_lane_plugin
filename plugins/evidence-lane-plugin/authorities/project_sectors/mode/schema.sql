...
CREATE TABLE mode_allowed_action(
...
CREATE TABLE mode_blocked_action(
                record_id INTEGER PRIMARY KEY,
                source_id INTEGER REFERENCES source_registry(source_id) ON DELETE CASCADE,
                locator TEXT NOT NULL,
                payload_json TEXT NOT NULL
...
CREATE VIRTUAL TABLE mode_fts USING fts5(
            path,
            locator,
            text_content,
            chunk_id UNINDEXED,
...
CREATE TABLE mode_response_template(
                record_id INTEGER PRIMARY KEY,
                source_id INTEGER REFERENCES source_registry(source_id) ON DELETE CASCADE,
                locator TEXT NOT NULL,
                payload_json TEXT NOT NULL
...
CREATE TABLE mode_supersede_ledger(
                record_id INTEGER PRIMARY KEY,
                source_id INTEGER REFERENCES source_registry(source_id) ON DELETE CASCADE,
                locator TEXT NOT NULL,
                payload_json TEXT NOT NULL
