# Custom sector package

This original source owner is `authorities/project_sectors/custom`. Live project state belongs to `sectors/custom` under the selected external project root. The packaged SQLite is an empty, unbound schema template; the engine creates identities and applies exact migrations under its writer. It is never opened as project state.
...
`schema.sql`, `migration-history.json` and `schema-template.json` describe the actual owner migrations. `custom.mmd` and `custom.dot` are matching schema traversal maps. `workflow.v4.json`, `workflow.mmd` and `workflow.dot` describe current action routes and their gates. They contain no project facts or runtime readiness assertion.
...
This package also serves registered `custom__<name>` instances. Sources owns each immutable adapter contract; every instance has its own `sectors/<lane_id>/` database, schema history, files and views. Use the existing Custom actions with exact lane and adapter selections; no project-specific code is loaded.
