# Local Code sector package

This original source owner is `authorities/project_sectors/local_code`. Live project state belongs to `sectors/local_code` under the selected external project root. The packaged SQLite is an empty, unbound schema template; the engine creates identities and applies exact migrations under its writer. It is never opened as project state.
...
`schema.sql`, `migration-history.json` and `schema-template.json` describe the actual owner migrations. `local_code.mmd` and `local_code.dot` are matching schema traversal maps. `workflow.v4.json`, `workflow.mmd` and `workflow.dot` describe current action routes and their gates. They contain no project facts or runtime readiness assertion.
