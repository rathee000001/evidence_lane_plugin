...
Current executable source package: `authorities/plan`. Storage: authorities/plan/plan_authority_v001.sqlite with files and schema history in authorities/plan. The package binds the canonical engine implementations listed in `runtime-binding.v4.json`.
...
`plan.sqlite` is a newly generated empty, unbound template. It contains no project identities, receipts or business records and is never opened as live state. `plan.mmd` and `plan.dot` describe its schema. The engine applies the actual owner migrations under a writer and records schema history in the selected external lane.
