# chat_lineage source owner

Current executable source package: `authorities/chat_lineage`. Storage: authorities/chat_lineage/chat_lineage_authority_v001.sqlite with files and schema history in authorities/chat_lineage. The package binds the canonical engine implementations listed in `runtime-binding.v4.json`.
...
`chat_lineage.sqlite` is a newly generated empty, unbound template. It contains no project identities, receipts or business records and is never opened as live state. `chat_lineage.mmd` and `chat_lineage.dot` describe its schema. The engine applies the actual owner migrations under a writer and records schema history in the selected external lane.
