"""Separate bounded evidence retrieval and project workflow configuration.

Retains the original deterministic slice ordering and workflow proposal behavior.
Workflow selection and work classification remain distinct; tool execution has
its owning actions.
"""
...
class ProjectEvidenceSelectionRequest(Contract):
    authority_id: str
    slices: list[EvidenceSlice] = Field(min_length=1, max_length=200)
...
class ProjectEvidenceSelectionResult(Contract):
    authority_id: str
    selected_slices: list[EvidenceSlice]
    selected_count: int
    available_count: int
    used_tokens: int
    token_budget: int
...
def select_project_evidence(request: ProjectEvidenceSelectionRequest):
    ...
    return ProjectEvidenceSelectionResult(authority_id=request.authority_id, selected_slices=selected,
...
class ProjectWorkflowRequest(Contract):
    ...
    work_classification_request: str | None = Field(default=None, min_length=1, max_length=4000)
...
class ProjectWorkflowResult(Contract):
    project_id: str
    batch_id: str
    batch_digest: str
    project_type: str
    ...
    workflow_and_work_classification_are_distinct: Literal[True] = True
    workflow_digest: str
...
_SOURCE_PROJECT_TYPES = {'local_code': 'CODE', 'github_code': 'CODE', 'research': 'RESEARCH',
    ...
    'images_ocr': 'MEDIA', 'ppt': 'DOCUMENT', 'docs': 'DOCUMENT', 'pdf_ocr': 'DOCUMENT', 'artifacts': 'MIXED'}


def _workflow_repository(source_root, mode, *, git_available):
    ...
    if mode == 'DISABLED' or not git_available:
        if mode == 'REQUIRED':
            raise LaneError('PROJECT_WORKFLOW_GIT_REQUIRED', 'The required Git tool is unavailable.')
        return observation
    ...
    def read(arguments):
        ...
        if remaining <= 0:
            raise LaneError('PROJECT_WORKFLOW_GIT_BUDGET', 'Repository observation exceeded its ten-second budget.')
    ...
    def required(message):
        if mode == 'REQUIRED':
            raise LaneError('PROJECT_WORKFLOW_GIT_REQUIRED', message, details={'repository_context': observation})
        return observation
    ...
    if len(lines) != 2 or lines[1] != 'true' or not Path(lines[0]).is_absolute():
        raise LaneError('PROJECT_WORKFLOW_GIT_RESPONSE', 'Git returned an unsupported worktree identity.')
    ...
    if branch.returncode not in (0, 1) or (head.returncode == 0 and not re.fullmatch(r'[0-9a-f]{40}|[0-9a-f]{64}', head.stdout.strip())):
        raise LaneError('PROJECT_WORKFLOW_GIT_RESPONSE', 'Git returned an invalid branch or commit identity.')
    ...
    if branch_name is not None and (not branch_name or '\n' in branch_name or len(branch_name) > 1024):
        raise LaneError('PROJECT_WORKFLOW_GIT_RESPONSE', 'Git returned an invalid branch identity.')
    if head.returncode != 0:
        ...
        if branch_name is None or read(['show-ref', '--verify', '--quiet', 'refs/heads/' + branch_name]).returncode != 1:
            raise LaneError('PROJECT_WORKFLOW_GIT_RESPONSE', 'The repository commit identity is unresolved.')
...
def _selected_workflows(registry, lanes, repository):
    ...
    if registry is None:
        raise LaneError('PROJECT_WORKFLOW_REGISTRY_REQUIRED', 'A workflow proposal requires the current executable action registry.')
    ...
    for lane in lanes:
        ...
        for action in actions:
            ...
            group = ('materialization_actions' if spec.materialization else 'read_actions' if not spec.mutates
                     else 'refresh_actions' if spec.workflow == 'refresh-project-evidence' else 'change_actions')
...
def compile_project_workflow(project, request: ProjectWorkflowRequest, *, registry=None, git_available=False):
    ...
    with project_snapshot(project.root), project.lane('sources').connection(read_only=True) as connection:
        if not connection.execute("SELECT 1 FROM sqlite_schema WHERE name='intake_batch'").fetchone():
            raise LaneError('SOURCE_BATCH_REQUIRED', 'Register the selected sources before deriving a project workflow.')
        ...
        if not lanes or not {lane_family(lane) for lane in lanes} <= set(SECTOR_LANE_IDS):
            raise LaneError('PROJECT_WORKFLOW_LANE_SET_INVALID', 'Workflows select retained sectors only.')
        if not {lane_family(lane) for lane in lanes} <= _SOURCE_PROJECT_TYPES.keys():
            raise LaneError('PROJECT_WORKFLOW_CLASS_REQUIRED', 'The registered lane requires an explicit current project-class mapping.')
        ...
        if any(value['selected_lanes_outside_class_defaults'] for value in lane_policies.values()):
            raise LaneError('PROJECT_WORKFLOW_CLASS_MISMATCH', 'The source class mapping differs from current locked lane policy.')
        work_classification = classify_operating_modes(
            request.work_classification_request or request.requested_outcome,
            explicit_modes=request.explicit_work_classes or None,
            code_lane=request.code_lane,
            custom_modes=request.custom_work_classes,
        ) if (
            request.explicit_work_classes
            or request.work_classification_request
            or request.custom_work_classes
        ) else None
        repository = _workflow_repository(project.source_root, request.git_mode, git_available=git_available)
        workflows, operations, execution = _selected_workflows(registry, lanes, repository)
        proposal = {'project_id': project.project_id, 'batch_id': request.batch_id, 'batch_digest': batch['batch_sha256'],
            ...
            'stage_sequence': ['source_intake', 'workflow_and_plan', 'delta_entry', 'lane_operations', 'verification', 'coordinated_delta_exit'],
            ...
            'selected_lane_validation': lane_policies, 'work_classification': work_classification,
        ...
        result = ProjectWorkflowResult(**proposal, workflow_digest=content_digest({**proposal, 'requested_outcome': request.requested_outcome}))
        if len(result.model_dump_json().encode('utf-8')) > request.max_result_bytes:
            raise LaneError('PROJECT_WORKFLOW_RESULT_BUDGET', 'The complete workflow proposal exceeds the selected result budget; increase it or select a smaller source batch.')
        return result


def register_first_class_actions(engine):
    from .tool_routes import ToolRoute
    engine.registry.register(ActionSpec('project_evidence_select', 'Select deterministic metadata slices using explicit estimated-token and item budgets; verify content through its owning authority.',
        ProjectEvidenceSelectionRequest, ProjectEvidenceSelectionResult, lambda context, request: select_project_evidence(request),
        workflow='retrieve-project-evidence', queryable_in_delta=True))
    def with_git(context, request):
        return compile_project_workflow(engine.directory.open(context.project_id), request,
            registry=engine.registry, git_available=True)

    def content_only(context, request):
        return compile_project_workflow(engine.directory.open(context.project_id), request, registry=engine.registry)

    engine.registry.register(ActionSpec('project_workflow_configure', 'Propose Git or content-only project workflows from a complete registered batch and current lane contracts without changing Plan or sources.',
        ProjectWorkflowRequest, ProjectWorkflowResult, with_git, workflow='configure-project-workflow', queryable_in_delta=True,
        tool_routes=(ToolRoute('project_workflow_configure.git', with_git, ('Python', 'Git'),
            argument_values=(('git_mode', ('AUTO', 'REQUIRED')),), reason='Bounded read-only observation of the exact source-root repository.'),
            ToolRoute('project_workflow_configure.content', content_only, ('Python',),
