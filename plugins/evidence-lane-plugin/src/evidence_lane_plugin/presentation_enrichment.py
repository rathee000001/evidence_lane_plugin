"""Optional, attributed Docling projections of immutable presentation snapshots."""
...
from .presentation_contracts import PresentationSnapshot
...
class PresentationEnrich(PresentationSnapshot):
    max_output_bytes: int = Field(default=1_048_576, ge=4096, le=2_097_152)


class PresentationEnrichmentRead(PresentationSelection):
...
def enrichment_worker(arguments):
    ...
    if digest(content) != arguments['expected_sha256']:
        raise LaneError('PRESENTATION_ENRICHMENT_INPUT_CHANGED', 'The enrichment bytes differ from the selected snapshot.')
    extension = Path(arguments['logical_name']).suffix.lower()
    if extension not in {'.pptx', '.potx', '.ppsx'}:
        raise LaneError('PRESENTATION_ENRICHMENT_FORMAT_UNSUPPORTED', 'The PPT enrichment operation accepts PPTX, POTX or PPSX snapshots.')
    with tempfile.TemporaryDirectory(prefix='evidence-lane-presentation-enrichment-') as folder:
        source = Path(folder) / ('presentation' + extension)
...
def enrich(context, request):
    ...
    lane = store.lane('ppt')
    response = execution.submit('presentation_enrich', {'logical_name': snapshot['logical_name'],
    ...
    if response['status'] != 'ok':
        raise LaneError(response.get('code', 'PRESENTATION_ENRICHMENT_FAILED'), 'The selected rich presentation converter did not complete.')
    ...
    if (digest(canonical_json_bytes(converted)) != receipt or converted['status'] != 'complete'
        ...
        raise LaneError('PRESENTATION_ENRICHMENT_BINDING', 'The rich conversion differs from its requested bytes or completion contract.')
    body = {'schema': 'evidence-lane.presentation-enrichment.v4', 'project_id': store.project_id, 'lane_id': 'ppt',
    ...
    with execution.lease.coordinated_transaction(['ppt', 'receipts']):
        ...
        with lane.transaction() as connection:
            ...
            connection.execute('INSERT INTO ppt_enrichment VALUES(?,?,?,?)', (identity, request.snapshot_id, obj, body['created_at']))
            store.append_receipt('presentation_enrichment', {'enrichment_id': identity, 'snapshot_id': request.snapshot_id,
                'engine': 'Docling', 'status': 'complete', 'source_bytes_mutated': False})
    return result(store, 'presentation_enrich', {'enrichment_id': identity, 'snapshot_id': request.snapshot_id,
...
def enrichment_manifest(store, identity):
    ...
    with lane.connection(read_only=True) as connection:
        row = connection.execute('SELECT * FROM ppt_enrichment WHERE enrichment_id=?', (identity,)).fetchone()
    if row is None:
        raise LaneError('PRESENTATION_ENRICHMENT_MISSING', 'Select an exact rich presentation conversion from this PPT lane.')
    ...
    if (digest(canonical_json_bytes(body)) != identity or body['project_id'] != store.project_id
            or body['lane_id'] != 'ppt' or body['snapshot_id'] != row['snapshot_id']):
        raise LaneError('PRESENTATION_ENRICHMENT_INTEGRITY', 'The rich presentation manifest differs from its indexed identity.')
    snapshot, _ = read_snapshot(store, body['snapshot_id'])
    if snapshot['raw_object'] != body['conversion']['source_sha256']:
        raise LaneError('PRESENTATION_ENRICHMENT_INTEGRITY', 'The rich conversion belongs to different source bytes.')
    return body
    ...
    def read(context, request):
        ...
        with project_snapshot(store.root):
            ...
            if structure is not None and len(canonical_json_bytes(structure)) > request.max_structure_bytes:
                raise LaneError('PRESENTATION_ENRICHMENT_READ_BUDGET', 'The structured projection exceeds this explicit result budget.')
            return result(store, 'presentation_enrichment_read', {'enrichment_id': request.enrichment_id,
