"""Gate MEDIA codecs behind owned processes with memory, time and output limits."""
...
from .media_parsers import digest, fail
from .process_ownership import ChildProcessGroup, ExtendedLimits, codec_python_executable, kernel


def invoke_media(operation, arguments, *, timeout_seconds=90):
    if operation not in {"parse", "transform", "ocr", "extract"}:
    ...
    if len(body) > 33_554_432:
        fail("INPUT_BUDGET")
    script = Path(__file__).with_name("media_child.py")
    ...
    try:
        if os.name == "nt":
            ...
            limits.basic.active_process_limit = (
                2  # One MEDIA child plus its selected native OCR tool.
            )
        ...
        if response.get("status") != "ok":
            ...
            if (
                isinstance(code, str)
                and code.startswith("MEDIA_")
                and len(code) < 80
                and code.replace("_", "").isalnum()
            ):
                raise LaneError(code, "The owned MEDIA operation rejected the requested input.")
            fail("NATIVE_OPERATION_FAILED")
    finally:
        group.close()
    ...
    result["process_evidence"] = {
        "owner": "engine_owned_media_child",
