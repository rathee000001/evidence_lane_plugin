"""Fixed MEDIA codec child. Input arrives only after its parent assigns ownership."""
...
def main():
    ...
    def audit(event, arguments):
        if event in {"socket.connect", "socket.connect_ex", "socket.getaddrinfo", "socket.bind"}:
            raise PermissionError("MEDIA codec network access is disabled")

    sys.addaudithook(audit)
    from evidence_lane_plugin.errors import LaneError
    from evidence_lane_plugin.media_workers import perform
    ...
    try:
        ...
        response = {"status": "error", "code": "MEDIA_NATIVE_OPERATION_FAILED"}
    output = json.dumps(
        response, ensure_ascii=True, separators=(",", ":"), allow_nan=False
    ).encode()
    if len(output) > 67_108_864:
        output = b'{"status":"error","code":"MEDIA_OUTPUT_BUDGET"}'
