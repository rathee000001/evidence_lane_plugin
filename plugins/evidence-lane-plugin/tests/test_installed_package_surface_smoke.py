...
def test_installed_mcp_stdio_catalog_is_ready_without_eager_ocr() -> None:
    messages = [
        {
            "jsonrpc": "2.0",
            "id": 1,
    ...
    process = subprocess.run(
        [sys.executable, str(PLUGIN / "scripts" / "run_mcp.py"), "--transport", "stdio"],
        input="".join(
...
def test_installed_schema_surface_is_complete_and_source_bound() -> None:
    ...
    assert manifest["public_action_schema_count"] == public["tool_count"] == NATIVE_TOOL_COUNT
...
def test_installed_sdk_contains_internal_env_uop_and_outer_routing_planes() -> None:
    ...
    assert len(list((PLUGIN / "sdk" / "workflows").glob("*.json"))) == GOVERNED_SKILL_COUNT
...
def test_installed_executable_tree_registry_hashes_every_declared_member() -> None:
    ...
    assert len({row["path"] for row in registry["members"]}) == registry[
        "member_count"
    ]
...
def test_installed_env_uop_are_single_executable_authorities() -> None:
    ...
    for relative, table in (
        ("env/env_sqlite.sqlite", "operator_activation_run"),
        ("uop/uop_sqlite.sqlite", "uop_delta_operator"),
    ):
        connection = sqlite3.connect(
...
def test_installed_eighteen_lanes_are_separate_complete_bindings() -> None:
    ...
    assert [row["lane_id"] for row in registry["lanes"]] == list(
        CANONICAL_LANE_IDS
    )
    ...
    for lane_id in CANONICAL_LANE_IDS:
        ...
        try:
            assert connection.execute("PRAGMA integrity_check").fetchone()[0] == "ok"
            assert dict(connection.execute("SELECT key, value FROM lane_meta"))[
                "lane_id"
            ] == lane_id
            assert connection.execute("SELECT COUNT(*) FROM source_registry").fetchone()[
                0
            ] == 0
        finally:
            connection.close()
...
def test_installed_hidden_runtime_contract_has_no_project_path_default() -> None:
    runtime = (PLUGIN / "scripts" / "runtime_contract.py").read_text(
        encoding="utf-8"
    )
    restart = (
        PLUGIN
...
def test_installed_toolchain_links_every_requirement_and_license_to_tunnel() -> None:
    ...
    assert {
        row["tunnel_prewarm_mode"] for row in tunnel["requirements"]
    } == {
