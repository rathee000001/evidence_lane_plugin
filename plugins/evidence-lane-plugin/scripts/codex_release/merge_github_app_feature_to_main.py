"""Merge one exact green feature commit to ``main`` through the GitHub App.

The command never checks out, edits, or directly pushes ``main``. GitHub's
repository merge endpoint reuses the existing feature tree, while the current
route verifies the exact source/target refs, required exact-head workflows,
resulting tree, ordered parents, App bot actor, and final ``main`` ref.
...
class GitHubAppMainMergeError(RuntimeError):
    """Raised before or during the governed feature-to-main merge."""
...
def _git(repository_root: Path, *args: str) -> str:
    ...
    if result.returncode != 0:
        raise GitHubAppMainMergeError(
            f"git {' '.join(args)} failed: {result.stderr.strip()}"
        )
    return result.stdout.strip().lower()


def local_main_merge_request(
    *,
    ...
    if current_branch != source_branch:
        raise GitHubAppMainMergeError(
            "GITHUB_APP_MAIN_MERGE_WRONG_LOCAL_BRANCH: the invoking worktree "
            "must remain attached to the exact governed feature branch."
        )
    if target_branch != "main" or source_branch == target_branch:
        raise GitHubAppMainMergeError(
            "GITHUB_APP_MAIN_MERGE_BOUNDARY_INVALID: only an exact non-main "
            "feature source may be merged into main."
        )
...
def execute_merge(
    *,
    ...
    request = local_main_merge_request(
    ...
    try:
        ...
        receipt = GitHubAppMainMergeRoute(
            broker=broker,
            transport=transport,
    ...
    if receipt_contains_secret(receipt):
        raise GitHubAppMainMergeError("The main-merge receipt contains a secret field.")
    output = receipt_path.resolve()
    content = canonical_json_bytes(receipt)
    output.parent.mkdir(parents=True, exist_ok=True)
    if output.exists() and output.read_bytes() != content:
        raise GitHubAppMainMergeError(
            "Refusing to overwrite another GitHub App main-merge receipt."
        )
