...
def _reexec(result: dict) -> None:
    if not result["reexec"]:
        return
    if os.name != "nt":
        os.execve(result["command"][0], result["command"], result["environment"])
    # Windows implements execve by creating another process and terminating the
    # caller. That breaks the anonymous stdio handles owned by an MCP client.
    # Keep this process as the transport parent and explicitly pass only the
    # three protocol streams to the validated installed command.
