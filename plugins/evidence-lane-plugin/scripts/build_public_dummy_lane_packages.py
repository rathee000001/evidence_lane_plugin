...
def _browser_executable() -> Path:
    ...
    browser = next((candidate.resolve() for candidate in candidates if candidate.is_file()), None)
    if browser is None:
        raise RuntimeError("An already-installed Chrome or Edge executable is required for Mermaid rendering.")
    return browser
...
def _mermaid_cli() -> str:
    ...
    raise RuntimeError("The already-installed Mermaid CLI is required; this command never installs it.")
...
def _artifact(path: Path, kind: str, label: str) -> dict[str, Any]:
    relative = path.relative_to(PLUGIN_ROOT / "remote_adapter" / "public").as_posix()
    return {
        "kind": kind,
        "label": label,
        "filename": path.name,
...
def build() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="evidence-lane-public-full-lanes-") as raw_temp:
        temporary = Path(raw_temp)
        ...
        if not validation["valid"]:
            raise RuntimeError(f"Full public lane bundle failed validation: {validation!r}")
        if result["emitted_lane_ids"] != list(CANONICAL_LANE_IDS) or result["omitted_lane_ids"]:
    ...
    if actual_lane_directories != set(CANONICAL_LANE_IDS):
        raise RuntimeError(f"Unexpected public lane directories: {actual_lane_directories!r}")
