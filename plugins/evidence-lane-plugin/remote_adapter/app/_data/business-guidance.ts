...
export const businessGuide: readonly BusinessGuideEntry[] = [
  ...
  },
  {
    ...
    answer: "Evidence Lane exposes 17 governed skill surfaces so users can see whether they are starting a session, bringing in evidence, building a candidate, refreshing changed material, recovering state, changing storage, exchanging bounded Canon work, managing project-isolated Agent Learning, or governing an optional connector. Six are everyday lifecycle controls: Boot, Rollback, Build, Refresh, Mode, and Source Intake. Canon and Agent Learning remain separate sidecars with separate authority; neither creates a hidden Project Truth or HIL promotion path.",
  ...
  },
  {
    ...
    answer: "Evidence Lane 2.2.0 is the current pre-HIL Codex source line. Accepted Project Truth remains PV12 on the 2.1 base; the enabled stable Git install is 2.1; and the disabled fallback was directly observed at 2.0. Those are four separate identities and none may be inferred from another. Codex lifecycle evidence must come from the package-local native route.\n\nStable installation of the exact 2.2 branch commit may occur only in its governed pre-HIL test route. Main promotion and replacement of the disabled fallback with the same accepted 2.2 package remain post-HIL actions. Evidence Lane currently governs only a positively proven Codex layer, regardless of which Beta or combined desktop shell exposes that layer. Google Drive and storage connectors are separate surfaces and never lifecycle proof.",
    href: "/connect",
  },
  {
    id: "persistent-goal",
  ...
  },
  {
    ...
    answer: "A valid 2.2.0 Codex test installation surface must show Evidence Lane, the correct icon, Praveen Rathee as the developer identity, exact version 2.2.0, a complete business description, the package-local MCP identity, 17 skills, 8 lifecycle hook events, and 83 native actions classified as 26 read and 57 write. The exact commit and package hashes must match the governed branch receipt.\n\nThat pre-HIL test install does not update accepted PV12, merge main, or replace the disabled fallback. Only fresh final PV14 Project HIL approval may authorize the exact accepted 2.2 commit to merge through the governed non-force route and enter both the enabled stable slot and disabled recoverable fallback slot. Any mismatched page is a hard stop.",
    href: "/connect",
  },
  {
    id: "connectors",
  ...
  },
  {
    ...
    keywords: ["release", "publish", "github", "website", "devpost", "merge main", "version 2.2.0"],
    answer: "The 2.2.0 pre-HIL route first aligns plugin source, runtime, package metadata, Git-tracked Markdown, every mapped website page, tests, and exact source identity. The governed branch commit then runs clean GitHub CI, CodeQL, dependency checks, exact-package verification, an authorized stable test install, and an automatic Vercel branch preview before the final six-way HIL.\n\nOnly after fresh exact approval may that same accepted commit merge to main through the governed non-force route, replace and verify the disabled fallback slot with the same 2.2 package, and proceed to separately authorized production publication. Preview success, CI success, installation, or continued conversation is never HIL approval.",
    href: "/proof",
  },
] as const;
