...
def three(system, tmp_path):
    ...
    participants = [successful(engine,store,client,'task_evidence_participant_register',{'label':f'Participant {i}'})['participant_id']
...
def prepare(three):
    ...
    expectation = successful(engine,b,cb,'task_evidence_expect',{'receiver_id':pb,'contract_key':'input',
        'sender_endpoints':[endpoint(a,pa)],'kinds':['requirements'],'auto_admit':True})
    original = successful(engine,a,ca,'task_evidence_send',{'sender_id':pa,'receiver_id':pb,'destination_project_id':b.project_id,
        'kind':'requirements','expected_contract':expectation['contract_digest'],'payload':{'summary':'Inspect an exact source input'}})
    incoming = successful(engine,b,cb,'task_evidence_receive',{'source_project_id':a.project_id,
        'exchange_id':original['exchange_id'],'envelope_digest':original['envelope_digest']})
    recipient_contract = successful(engine,c,cc,'task_evidence_expect',{'receiver_id':pc,'contract_key':'backfire',
        'sender_endpoints':[endpoint(b,pb)],'kinds':['backfire'],'auto_admit':True})
    return_contract = successful(engine,a,ca,'task_evidence_expect',{'receiver_id':pa,'contract_key':'backfire_return',
...
def test_exact_third_party_backfire_and_declared_return_remain_receiver_owned(three):
    ...
    raised = successful(engine,b,cb,'task_evidence_input_request',args)
    ...
    preview = successful(engine,c,cc,'task_evidence_packet_classify',backfire_locator)
    assert preview['classification'] == 'expected' and preview['automatic_admission_permitted']
    assert (snapshot(b),snapshot(a)) == peer_before
    received = successful(engine,c,cc,'task_evidence_receive',backfire_locator)
    assert received['state'] == 'admitted' and (snapshot(b),snapshot(a)) == peer_before
    inbox = successful(engine,c,cc,'task_evidence_inbox',{'exchange_id':received['exchange_id']})
    assert inbox['packets'][0]['backfire'] == details
    # C returns to the explicitly declared A, which never received B's packet.
    returned = successful(engine,c,cc,'task_evidence_send',{'sender_id':pc,'receiver_id':pa,'destination_project_id':a.project_id,
    ...
    before_c = snapshot(c)
    admitted = successful(engine,a,ca,'task_evidence_receive',{'source_project_id':c.project_id,
    ...
    assert successful(engine,b,cb,'task_evidence_inspect')['objects_verified']['conditional_operations'] == 1
    assert successful(engine,a,ca,'task_evidence_inspect')['foreign_key_errors'] == []
...
def test_backfire_dedup_preserves_original_bytes_and_reserves_replay_request_ids(three):
    ...
    _, args = prepare(three)
    first = successful(engine,b,cb,'task_evidence_input_request',args)
    assert not first['duplicate']
    alias = {**args,'request_id':str(uuid4()),'expires_at':(datetime.now(UTC)+timedelta(hours=2)).isoformat()}
    repeated = successful(engine,b,cb,'task_evidence_input_request',alias)
    ...
    assert successful(engine,b,cb,'task_evidence_input_request',alias)['duplicate']
    ...
    assert call(engine,b,cb,'task_evidence_input_request',conflict).error.code == 'CANON_BACKFIRE_DEDUP_CONFLICT'
    assert call(engine,b,cb,'task_evidence_input_request',{**conflict,'request_id':alias['request_id']}).error.code == 'CANON_REQUEST_CONFLICT'
...
def test_only_the_admitted_input_owner_can_raise_each_retained_backfire_class(three, failure):
    ...
    assert call(engine,b,ca,'task_evidence_input_request',args).error.code == 'CANON_OWNER_MISMATCH'
    assert successful(engine,b,cb,'task_evidence_input_request',args)['state'] == 'sealed'


def test_backfire_requires_current_admission_revision_exact_digest_and_acyclic_trace(three):
    engine, (a,b,c), (ca,cb,cc), (pa,pb,pc) = three
    original, args = prepare(three)
    assert call(engine,b,cb,'task_evidence_input_request',{**args,'admitted_envelope_digest':'0'*64}).error.code == 'CANON_BACKFIRE_INPUT_MISMATCH'
    assert call(engine,b,cb,'task_evidence_input_request',{**args,'trace':[endpoint(c,pc)]}).error.code == 'CANON_BACKFIRE_ROUTE_CYCLE_BLOCKED'
    assert call(engine,b,cb,'task_evidence_input_request',{**args,'trace':[endpoint(b,pb)]}).error.code == 'CANON_BACKFIRE_ROUTE_CYCLE_BLOCKED'
    assert call(engine,b,cb,'task_evidence_input_request',{**args,'dependency_ids':[str(uuid4())]}).error.code == 'CANON_EDGE_NOT_FOUND'
    proposed = successful(engine,b,cb,'task_evidence_input_request',args)
    ...
    correction = successful(engine,a,ca,'task_evidence_send',{'sender_id':pa,'receiver_id':pb,'destination_project_id':b.project_id,
        'kind':'correction','supersedes':original['exchange_id'],'payload':{'summary':'A corrected exact source input'}})
    pending = successful(engine,b,cb,'task_evidence_receive',{'source_project_id':a.project_id,
    ...
    assert call(engine,b,cb,'task_evidence_input_request',updated).error.code == 'CANON_BACKFIRE_REQUIRES_ADMITTED_INPUT'
    successful(engine,b,cb,'task_evidence_decide',{'exchange_id':pending['exchange_id'],'envelope_digest':pending['envelope_digest'],
    ...
    assert call(engine,b,cb,'task_evidence_input_request',args).error.code == 'CANON_BACKFIRE_REQUIRES_ADMITTED_INPUT'
    locator = {'source_project_id':b.project_id,'exchange_id':proposed['exchange_id'],'envelope_digest':proposed['envelope_digest']}
    preview = successful(engine,c,cc,'task_evidence_packet_classify',locator)
    assert preview['classification'] == 'undefined_or_incompatible' and preview['reasons'] == ['CANON_BACKFIRE_REQUIRES_ADMITTED_INPUT']
    assert call(engine,c,cc,'task_evidence_receive',locator).error.code == 'CANON_BACKFIRE_REQUIRES_ADMITTED_INPUT'
    assert call(engine,b,cb,'task_evidence_input_request',updated).error.code == 'CANON_BACKFIRE_STALE_REVISION_BLOCKED'
    assert successful(engine,b,cb,'task_evidence_input_request',{**updated,'requested_revision':3})['state'] == 'sealed'
...
def test_generic_backfire_cannot_bypass_typed_conditional_guards(three):
    ...
    assert call(engine,b,cb,'task_evidence_send',message).error.code == 'CANON_TYPED_BACKFIRE_REQUIRED'
    preview = successful(engine,b,cb,'task_evidence_classify',message)
...
def test_backfire_send_and_semantic_record_rollback_together(three, monkeypatch):
    ...
    with monkeypatch.context() as patch:
        ...
        assert call(engine,b,cb,'task_evidence_input_request',args).status == 'error'
    with b.lane('canon').connection(read_only=True) as db:
    ...
    assert not successful(engine,b,cb,'task_evidence_input_request',args)['duplicate']
...
def test_hash_consistent_dedup_record_cannot_attest_different_semantics(three):
    ...
    assert CanonStore(b).verify_history()['events_verified'] > 0
    assert call(engine,b,cb,'task_evidence_input_request',args).error.code == 'CANON_OPERATION_INTEGRITY'
    assert call(engine,b,cb,'task_evidence_inspect').error.code == 'CANON_OPERATION_INTEGRITY'
    assert call(engine,c,cc,'task_evidence_receive',{'source_project_id':b.project_id,
...
def test_receiver_cannot_restart_a_backfire_by_omitting_its_inherited_trace(three):
    ...
    _, args = prepare(three)
    raised = successful(engine,b,cb,'task_evidence_input_request',args)
    incoming = successful(engine,c,cc,'task_evidence_receive',{'source_project_id':b.project_id,
    ...
    assert call(engine,c,cc,'task_evidence_input_request',attempted).error.code == 'CANON_BACKFIRE_ROUTE_CYCLE_BLOCKED'
...
def test_local_inbox_omits_operation_payload_and_preserves_each_packets_trace(three):
    ...
    _, args = prepare(three)
    peer = successful(engine,b,ca,'task_evidence_participant_register',{'label':'Local upstream recipient'})['participant_id']
    expected = successful(engine,b,ca,'task_evidence_expect',{'receiver_id':peer,'contract_key':'local_backfire',
        'sender_ids':[pb],'kinds':['backfire'],'fields':{'content':'string'},'auto_admit':True})
    returning = successful(engine,b,cb,'task_evidence_expect',{'receiver_id':pb,'contract_key':'local_return',
    ...
    packets = [successful(engine,b,cb,'task_evidence_input_request',{**args,'request_id':str(uuid4()),'requested_revision':revision})
        for revision in (2,3)]
    page = successful(engine,b,ca,'task_evidence_inbox',{'receiver_id':peer})
...
def test_result_dedup_uses_edge_payload_and_conflicts_on_another_input(projects):
    ...
    target = successful(engine,b,cb,'task_evidence_expect',{'receiver_id':pb,'contract_key':'input',
        'sender_endpoints':[endpoint(a,pa)],'kinds':['requirements'],'auto_admit':True})
    return_contract = successful(engine,a,ca,'task_evidence_expect',{'receiver_id':pa,'contract_key':'return',
        'sender_endpoints':[endpoint(b,pb)],'kinds':['result'],'auto_admit':True})
    edge = successful(engine,a,ca,'task_evidence_edge_register',{'source_id':pa,'destination':endpoint(b,pb),
    ...
    successful(engine,b,cb,'task_evidence_edge_bind',{'source_project_id':a.project_id,'edge_id':edge['edge_id'],'edge_digest':edge['edge_digest']})
    inputs = []
    for number in (1,2):
        sealed = successful(engine,a,ca,'task_evidence_send',{'sender_id':pa,'receiver_id':pb,'destination_project_id':b.project_id,
        ...
        inputs.append(successful(engine,b,cb,'task_evidence_receive',{'source_project_id':a.project_id,
    ...
    first = successful(engine,b,cb,'task_evidence_result',args)
    alias = {**args,'request_id':str(uuid4()),'expires_at':(datetime.now(UTC)+timedelta(hours=2)).isoformat()}
    replay = successful(engine,b,cb,'task_evidence_result',alias)
    assert replay['duplicate'] and replay['envelope_digest'] == first['envelope_digest']
    assert successful(engine,b,cb,'task_evidence_result',alias)['duplicate']
    assert call(engine,b,ca,'task_evidence_result',alias).error.code == 'CANON_OWNER_MISMATCH'
    assert call(engine,b,cb,'task_evidence_result',{**args,'request_id':str(uuid4()),'reply_to':inputs[1]['exchange_id']}).error.code == 'CANON_TASK_RESULT_DEDUP_CONFLICT'
    assert successful(engine,b,cb,'task_evidence_inspect')['objects_verified']['conditional_operations'] == 1
