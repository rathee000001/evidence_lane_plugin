"""Repository-only PoC: build eighteen lanes with one exact real Git lane.
...
def build_poc(repository: Path, output: Path, ref: str, subject: str) -> dict[str, Any]:
    ...
    readme = f"""# Evidence Lane v3.0.0 exact-Git PoC
...
def main() -> int:
    ...
    parser.add_argument(
        "--subject",
        default="Evidence Lane v3.0.0 exact plugin commit and seventeen fixture lanes",
    )
