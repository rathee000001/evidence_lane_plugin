"""Repository-only PoC: build and audit a deterministic eighteen-lane package.
...
def build_one_shot_dummy_poc(output: Path, subject: str) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="evidence-lane-one-shot-dummy-") as temp:
        ...
        # Some native parsers and Windows directory iterators release their
        # repository handles only when their short-lived wrapper objects are
        # finalized.  Make that boundary explicit before TemporaryDirectory
        # removes the synthetic repository; never hide a cleanup failure.
        gc.collect()
