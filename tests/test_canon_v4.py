...
def pair(system):
    ...
    first=invoke(sender,'task_evidence_participant_register',CanonJoin(label='Source task',reported_host_task_id=str(uuid4())))
    second=invoke(receiver,'task_evidence_participant_register',CanonJoin(label='Receiver task'))
...
def send(pair,**changes):
    ...
    result=invoke(sender,'task_evidence_send',request)
...
def decide(pair,result,decision='admit',**changes):
    ...
    response=invoke(receiver,'task_evidence_decide',request)
...
def test_sender_cannot_impersonate_receiver_and_admission_does_not_change_plan(pair):
    ...
    assert invoke(sender,'task_evidence_send',request).result['duplicate'] is True
    forged=request.model_copy(update={'request_id':str(uuid4()),'sender_id':target,'receiver_id':source})
    assert invoke(sender,'task_evidence_send',forged).error.code=='CANON_OWNER_MISMATCH'
    ...
    assert invoke(sender,'task_evidence_decide',decision).error.code=='CANON_OWNER_MISMATCH'
    admitted=invoke(receiver,'task_evidence_decide',decision)
    ...
    assert invoke(receiver,'task_evidence_decide',decision).result==admitted.result
...
def test_receiver_expected_contract_is_typed_versioned_and_exact(pair):
    ...
    assert invoke(sender,'task_evidence_expect',contract).error.code=='CANON_OWNER_MISMATCH'
    registered=invoke(receiver,'task_evidence_expect',contract)
    ...
    accepted=invoke(sender,'task_evidence_send',good)
    ...
    incompatible=invoke(sender,'task_evidence_send',wrong)
    ...
    assert invoke(receiver,'task_evidence_decide',missing_accept).error.code=='CANON_INPUT_ACCEPT_REQUIRED'
    changed=contract.model_copy(update={'request_id':str(uuid4()),'expected_version':1,'active':False})
    assert invoke(receiver,'task_evidence_expect',changed).result['version']==2
    stale=invoke(sender,'task_evidence_send',good.model_copy(update={'request_id':str(uuid4())}))
    assert stale.result['state']=='received' and stale.result['compatibility_reasons']==['CANON_CONTRACT_MISMATCH']
    assert invoke(receiver,'task_evidence_expect',contract.model_copy(update={'request_id':str(uuid4())})).error.code=='CANON_CONTRACT_VERSION_CONFLICT'
...
def test_pending_correction_preserves_original_until_receiver_admission(pair):
    ...
    assert invoke(sender,'task_evidence_send',bad).error.code=='CANON_CORRECTION_MISMATCH'
...
def test_result_backfire_return_route_and_clarification_keep_ownership(pair):
    ...
    assert invoke(receiver,'task_evidence_send',response).error.code=='CANON_TYPED_BACKFIRE_REQUIRED'
    response=response.model_copy(update={'request_id':str(uuid4()),'kind':'clarification'})
    assert invoke(receiver,'task_evidence_send',response).error.code=='CANON_RETURN_REQUIRES_INPUT'
    decide(pair,original,decision='clarify')
    returned=invoke(receiver,'task_evidence_send',response)
    ...
    assert invoke(sender,'task_evidence_send',wrong).error.code=='CANON_RETURN_ROUTE_MISMATCH'
    ...
    assert invoke(sender,'task_evidence_send',expired).error.code=='CANON_EXCHANGE_EXPIRED'
...
def test_evidence_checks_and_failed_write_rollback_are_atomic(pair,monkeypatch):
    ...
    assert invoke(sender,'task_evidence_send',bad).error.code=='MEMORY_SOURCE_MISMATCH'
    store=system[1]
    ...
    response=invoke(sender,'task_evidence_send',request)
...
def test_memory_references_keep_canon_state_and_queries_are_read_only(pair):
    ...
    page=invoke(receiver,'task_evidence_read',{'include_payload':True})
...
def test_envelope_integrity_and_byte_budget(pair):
    ...
    assert invoke(sender,'task_evidence_read',{}).error.code=='CANON_ENVELOPE_INTEGRITY'


def test_return_contract_is_pinned_to_source_expectation_and_chain_is_bounded(pair):
    invoke,sender,receiver,source,target,_system=pair
    registered=invoke(sender,'task_evidence_expect',CanonExpected(receiver_id=source,contract_key='return_value',sender_ids=[target],kinds=['result'],fields={'ok':'boolean'},auto_admit=True))
    ...
    assert invoke(receiver,'task_evidence_send',result).error.code=='CANON_RETURN_CONTRACT_MISMATCH'
    response=invoke(receiver,'task_evidence_send',result.model_copy(update={'expected_contract':digest}))
    ...
    for depth in range(2,9):
        ...
        last=invoke(actor,'task_evidence_send',req)
        assert last.status=='ok',last.error
        accepted=invoke(recipient,'task_evidence_decide',CanonDecide(exchange_id=last.result['exchange_id'],envelope_digest=last.result['envelope_digest'],expected_version=1,decision='admit',reason='Continue bounded clarification',incompatible_input_decision='ACCEPT'))
        assert accepted.status=='ok'
        from_id,to_id,actor,recipient=to_id,from_id,recipient,actor
    over=invoke(actor,'task_evidence_send',CanonSend(sender_id=from_id,receiver_id=to_id,kind='clarification',reply_to=last.result['exchange_id'],payload=CanonPayload(summary='Ninth return')))
...
def test_state_version_corruption_is_rejected_before_admission(pair):
    ...
    assert invoke(sender,'task_evidence_read',{}).error.code=='CANON_STATE_INTEGRITY'
