...
def initialize(source, *, commit=True):
    git(source, 'init', '-b', 'codex/recipe-fixture')
    git(source, 'config', 'user.name', 'Recipe fixture')
    git(source, 'config', 'user.email', 'recipe@example.invalid')
    git(source, 'config', 'core.autocrlf', 'false')
    if commit:
        git(source, 'add', 'app.py')
        git(source, 'commit', '-m', 'Recipe fixture')
...
def system(tmp_path, source=None):
    ...
    with Engine(tmp_path / 'runtime') as engine:
        ...
        def recipe(**values):
            return call('project_recipe', {'batch_id': batch, 'requested_outcome': 'Maintain the selected source.', **values})

        yield engine, store, call, recipe


@pytest.mark.parametrize('state', ['attached', 'detached', 'unborn', 'linked'])
def test_exact_git_recipe_preserves_every_source_and_database_byte(tmp_path, state):
    ...
    if state == 'linked':
        ...
        git(source, 'worktree', 'add', '-b', 'codex/linked-recipe', str(linked))
        source = linked
    if state != 'unborn':
    ...
    with system(tmp_path, source) as (_, store, _, recipe):
        before = files(tmp_path / 'source'), files(source), database_bytes(store)
        response = recipe(git_mode='REQUIRED')
        ...
        assert response.tool_execution['selection']['selected_route'] == 'project_recipe.git'
...
def test_non_git_and_parent_repository_do_not_get_adopted(tmp_path, monkeypatch):
    ...
    with tempfile.TemporaryDirectory(prefix='evi-recipe-no-git-', dir=system_temp) as temporary, system(tmp_path, Path(temporary)) as (_, store, _, recipe):
        ...
        required = recipe(git_mode='REQUIRED')
        assert required.error.code == 'PROJECT_RECIPE_GIT_REQUIRED'
    ...
    with system(nested_runtime, child) as (_, store, _, recipe):
        ...
        assert result.result['execution_workflow']['kind'] == 'content_only'
        assert recipe(git_mode='REQUIRED').error.code == 'PROJECT_RECIPE_GIT_REQUIRED'
...
def test_real_missing_git_uses_content_fallback_but_required_fails(tmp_path, monkeypatch):
    with system(tmp_path) as (_, store, _, recipe):
        before = files(store.source_root), database_bytes(store)
        ...
        assert result.tool_execution['selection']['selected_route'] == 'project_recipe.content'
...
def test_git_failure_after_invocation_does_not_silently_switch_to_content(tmp_path, monkeypatch):
    ...
    with system(tmp_path) as (_, store, _, recipe):
        ...
        assert response.error.details['tool_execution']['selection']['selected_route'] == 'project_recipe.git'
        assert not response.error.details['tool_execution']['automatic_retry']
        assert (files(store.source_root), database_bytes(store)) == before


def test_mixed_recipe_keeps_all_lane_contracts_and_refuses_partial_output(tmp_path):
    with system(tmp_path) as (engine, store, call, _):
        ...
        before = files(store.source_root), database_bytes(store)
        result = call('project_recipe', arguments)
        assert result.status == 'ok', result.error
        body = result.result
        ...
        assert not body['source_bytes_reverified'] and body['mode_classification'] is None
        for name, contract in body['operation_contracts'].items():
            assert engine.registry.get(name).schema()['toolchain'] == contract['toolchain']
        denied = call('project_recipe', {**arguments, 'max_result_bytes': 8192})
        assert denied.error.code == 'PROJECT_RECIPE_RESULT_BUDGET'
        assert (files(store.source_root), database_bytes(store)) == before


@pytest.mark.parametrize('git_enabled', [False, True])
def test_packaged_recipe_prepares_and_executes_a_verified_code_task(tmp_path, git_enabled):
    source = tmp_path / 'source'
    source.mkdir()
    ...
    with create_runtime_engine(tmp_path / 'runtime', workers=1, capabilities=CapabilityMonitor()) as engine:
        ...
        async def run():
            async with native(engine.root, store.project_id, permissions=('read', 'write', 'tools'), entrypoint='package') as session:
                ...
                assert tools['project_recipe'].annotations.readOnlyHint
                ...
                recipe = await call('project_recipe', {'batch_id': registered['source_authority']['batch_id'],
                    'requested_outcome': 'Index the selected Python function.', 'git_mode': 'AUTO'})
                assert recipe['execution_workflow']['kind'] == ('git_existing_project' if git_enabled else 'content_only')
                assert (files(source), database_bytes(store)) == before
                prepared = (await call(recipe['lane_workflows'][0]['source_preparation_action'],
                ...
                assert task['operation']['action'] in recipe['lane_workflows'][0]['materialization_actions']
