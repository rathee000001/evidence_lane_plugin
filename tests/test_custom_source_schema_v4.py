...
@pytest.mark.parametrize('lane', ['plan', 'chat_lineage', 'memory', 'sources'])
def test_custom_source_schemas_cannot_target_authorities(lane):
    definition = _definition()
    definition['target_lane'] = lane
    with pytest.raises(EvidenceLaneError) as error:
        compile_custom_source_schema(definition)
    assert error.value.code == 'CUSTOM_SOURCE_SCHEMA_TARGET_LANE_INVALID'
    definition = _definition()
    definition['selectors'][0]['lane_ids'] = [lane]
    with pytest.raises(EvidenceLaneError) as error:
        compile_custom_source_schema(definition)
    assert error.value.code == 'CUSTOM_SOURCE_SCHEMA_LANE_INVALID'
