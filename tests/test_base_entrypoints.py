...
def test_public_routes_derive_from_the_started_engine_registry(tmp_path):
    ...
    with Engine(tmp_path) as engine:
        ...
        assert all(route["implementation"] == "evidence_lane_plugin.sdk.dispatch" for route in routes["routes"])
...
def test_corrected_imported_cli_runs_native_mcp_to_same_engine(tmp_path):
    ...
    async def exercise():
        ...
        async with (stdio_client(parameters) as (read, write),
            ...
            assert initialized.serverInfo.version == "4.0.8"
...
def test_bound_package_launcher_owns_installed_reentry() -> None:
    plugin = Path(__file__).parents[1] / "plugins/evidence-lane-plugin"
    ...
    assert binding["release_ref"] == (
        "refs/tags/evidence-lane-v4.0.8-bundle-977fb5ec2702ff9d"
    )
