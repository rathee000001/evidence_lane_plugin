...
export type OpticalRouteDesign={bands?:number;threads?:number;colors?:number[];width?:number;spacing?:number;threadSpacing?:number;opacity?:number;threadOpacity?:number;motion?:number};
 ...
 */
export function createHomeOpticalRoute(T:typeof Three,parent:Three.Object3D,segments=96,scale=1,design:OpticalRouteDesign={}){
 const bandCount=design.bands??3,threadCount=design.threads??7,colors=design.colors??[0x8bd5fa,0xb7a1ed,0xf0d4ab],opacity=design.opacity??.48,threadOpacity=design.threadOpacity??.45,motion=design.motion??1;
 const across=5,centers=new Float32Array((segments+1)*3),arc=new Float32Array(segments+1),point=new T.Vector3();
 const materials=Array.from({length:bandCount},(_,i)=>new T.MeshPhysicalMaterial({color:colors[i%colors.length],metalness:.08,roughness:.055,transparent:true,opacity,transmission:.6,thickness:.16,ior:1.42,clearcoat:1,clearcoatRoughness:.04,side:T.DoubleSide,depthWrite:false,envMapIntensity:1.3,iridescence:.65,iridescenceIOR:1.3}));
 ...
 const beams=Array.from({length:threadCount},(_,j)=>{
  ...
  const material=new T.LineBasicMaterial({color:colors[j%colors.length],transparent:true,opacity:threadOpacity,blending:T.AdditiveBlending,depthWrite:false});
 ...
 const pointAt=(t:number,target:Three.Vector3)=>{sampler(t,target);const envelope=Math.sin(Math.PI*t)**2;target.y+=Math.sin(target.x*1.9+clock*.9)*.055*envelope*routeScale*motion;target.z+=Math.cos(target.x*1.4-clock*.7)*.07*envelope*routeScale*motion;return target};
 return{
  pointAt,
  update(time:number,sample:(t:number,target:Three.Vector3)=>void,alpha=1){
   clock=time;sampler=sample;
   strips.forEach((strip,j)=>{strip.mesh.visible=alpha>.005;materials[j].opacity=opacity*alpha});beams.forEach((beam,j)=>{beam.line.visible=alpha>.005;beam.material.opacity=threadOpacity*(.82+Math.sin(time*.8+j)*.18)*alpha});
   if(alpha<=.005)return;
   ...
   for(let i=0;i<=segments;i++){
    ...
    for(let j=0;j<bandCount;j++){
     const strip=strips[j],twist=.32*Math.sin(a*2+time*.16+j*.3),cx=Math.cos(twist),sz=-Math.sin(twist),width=(design.width??(j===1?.21:.115))*(.78+.22*Math.sin(a*3+time*.18+j*.9)**2)*routeScale;
     const wx=nx*cx,wy=ny*cx,wz=sz,n0=dy*wz-dz*wy,n1=dz*wx-dx*wz,n2=dx*wy-dy*wx,norm=Math.hypot(n0,n1,n2)||1;
     for(let k=0;k<=across;k++){const at=(i*(across+1)+k)*3,offset=((k/across-.5)*width+(j-(bandCount-1)/2)*(design.spacing??.12)*routeScale)*envelope;
      strip.positions[at]=centers[i*3]+wx*offset;strip.positions[at+1]=centers[i*3+1]+wy*offset;strip.positions[at+2]=centers[i*3+2]+wz*offset-(j-(bandCount-1)/2)*.035*routeScale*envelope;
      strip.normals[at]=-n0/norm;strip.normals[at+1]=-n1/norm;strip.normals[at+2]=-n2/norm;
     }
    }
    beams.forEach((beam,j)=>{const lane=j-(threadCount-1)/2,at=i*3,spread=lane*(design.threadSpacing??.056)*(.94+.06*Math.sin(a*2+time*.3))*envelope*routeScale;beam.positions[at]=centers[at]+nx*spread;beam.positions[at+1]=centers[at+1]+ny*spread;beam.positions[at+2]=centers[at+2]+lane*.025*Math.sin(a*2+time*.16)*envelope*routeScale});
   }
