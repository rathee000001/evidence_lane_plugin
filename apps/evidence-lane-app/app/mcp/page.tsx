...
export const metadata: Metadata = {
  title: "Native MCP",
  description: `Evidence Lane's package-local native MCP server, ${currentProductContract.nativeMcp.totalActions} governed actions, and ${currentProductContract.ordinaryLiveAuthorityCount} separate live authorities.`,
};

const actionClasses = [
  [String(currentProductContract.nativeMcp.readActions), "read-only actions", "Inspect accepted pointers, status, Plan, receipts, sources, lanes, Canon, Learning, and governed evidence without lifecycle mutation.", "database"],
  [String(currentProductContract.nativeMcp.writeActions), "write-capable actions", "Mutate only through explicit project/session contracts, exact task identity, one-writer guards, and operation-specific receipts.", "terminal"],
...
export default function McpPage() {
  return (
    <main>
      <PageHero
        eyebrow="Native MCP"
        title={`One package-local server. ${currentProductContract.nativeMcp.totalActions} governed actions.`}
      ...
      <section className="section architectureDark"><div className="shell"><div className="sectionHead wideHead"><span className="kicker light">Transport boundary</span><h2>The website explains the MCP. It never becomes the MCP.</h2><p>This page consumes the fingerprinted read-only projection of the canonical public-action catalog. It exposes {currentProductContract.ordinaryLiveAuthorityCount} separate ordinary live authorities while Project Overlay remains HIL-only. A successful page render, preview deployment, or public response cannot prove native plugin pickup, invoke lifecycle state, or move an accepted pointer.</p></div></div></section>
    </main>
  );
}
