export type PriorExecutionPlanStatus = "COMPLETED" | "IN_PROGRESS" | "PENDING";

export type ExecutionPlanRow = {
  number: number;
  status: PriorExecutionPlanStatus;
  step: string;
};
...
export const priorExecutionPlan: readonly ExecutionPlanRow[] = steps.map((step, index) => ({
  number: index + 1,
  status: index < 45 ? "COMPLETED" : index === 45 ? "IN_PROGRESS" : "PENDING",
  step,
}));

export const priorAuthoritativeCorrection =
...
export const priorExecutionPlanBoundary = {
