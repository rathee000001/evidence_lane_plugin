...
class LaneReader:
    ...
    def search_parallel(
        ...
        *,
                ...
                while pending:
                    ...
                    for future in completed:
                        ...
                        if worker["kind"] == "ERROR":
                            error = dict(worker["error"])
                            request_receipts.append(
                                {
                                    **job,
                                    "status": str(error.get("status") or "FAIL"),
                                    "within_time_budget": True,
                                    "result_count": 0,
                                    "result_sha256": None,
                                    "error": error,
        ...
        for receipt in request_receipts:
            result = lane_results.get(str(receipt["request_sha256"]))
            if result is None:
                continue
            result_project_id = str(result.get("project_id") or project_id)
            result_pv_ref = str(result.get("pv_ref") or resolved_pv_ref)
            lane_id = str(
                (result.get("lane") or {}).get("canonical_lane_id")
    ...
    def _cross_project_permission(
        ...
        *,
        ...
        grant = self.cross_project_authorizer(request)
        require(
    ...
    def search_cross_project(
        ...
        *,
        ...
        for project_index, raw in enumerate(project_queries):
            ...
            for lane_index, lane_query in enumerate(raw_lane_queries):
                require(
