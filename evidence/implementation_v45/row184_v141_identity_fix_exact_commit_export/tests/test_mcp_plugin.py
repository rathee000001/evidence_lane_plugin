...
const windowObject = {{
  ...
  openai: {{
    ...
    toolOutput: {{
      ...
      status: "PASS",
      facts: [{{ label: "Release", value: "1.4.1" }}],
      lanes: [],
      links: [],
    }},
...
if (init.message.params.appInfo.version !== "1.4.1") throw new Error("wrong app version");
...
def test_mcp_server_advertises_exact_release_and_cube_icon(tmp_path: Path) -> None:
    ...
    assert identity.version == ENGINE_VERSION == "1.4.1"
...
def test_real_stdio_chatgpt_pro_profile_has_complete_visible_inventory(
    ...
    async def exercise() -> None:
        ...
        async with (
            ...
            assert initialized.serverInfo.version == ENGINE_VERSION == "1.4.1"
            tools = await session.list_tools()
