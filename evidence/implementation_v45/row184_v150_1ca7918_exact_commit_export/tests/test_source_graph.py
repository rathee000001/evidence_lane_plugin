...
def test_service_graph_receipt_preserves_session_and_pointer(service, tmp_path: Path) -> None:
    ...
    receipt = register_source_batch(
        registry,
        [SourceAuthoritySpec(str(project), 1, "local_code")],
    )
    service.sessions.classify(
        "book-faires",
        session_id,
        task_class="research",
        requested_outcome="Build one bounded registered source graph.",
        permitted_paths=[],
        permitted_tools=["repository_read"],
        acceptance_checks=[],
        stop_condition="Stop after the graph receipt is recorded.",
    )
    ...
    result = service.source_graph_build(
        "book-faires",
        str(receipt["batch_id"]),
