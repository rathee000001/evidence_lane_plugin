...
- `active_pointer.json` remains compare-and-swap lifecycle metadata outside the
  accepted artifact.
- `accepted/` contains the currently accepted artifact. An existing unpacked
  `PV12/` remains valid legacy evidence until a later exact Project HIL.
...
Legacy directory-backed accepted PVs remain readable and valid. The new store
adapters expose one validation/metadata interface for legacy directories and
content-addressed ZIPs, while temporary accepted views are materialized outside
the project authority and removed after use.
