...
Register or inspect persistent local project state and its separate lane databases. Use for local project root selection.

## What this helps you do

Distinct source and state roots with separately owned project records.

## Use it when

Use **Choose project storage** when your intention matches this example:

> Register this source folder with a separate durable project-state folder.
...
1. **Register the exact project roots.** Register an existing project root or create one direct flat project-state root, intake its selected source lane first, then initialize retained authorities without unrelated sectors. This stage changes recorded project state.
2. **Inspect storage observations.** Read distinct source, state, engine and plugin roots and the exact published lane references. This stage is read-only.
...
Persistent local Windows only. Registering roots does not migrate existing data or prove that source indexing has run.
...
- Public skill: `select-project-storage`
- Representative actions: `project_register`, `storage_status`
- Some representative stages can change project records: `project_register`.
