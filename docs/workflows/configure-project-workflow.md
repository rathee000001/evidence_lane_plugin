...
A proposal tying the requested outcome to source types and relevant checks.

## Use it when

Use **Shape the project workflow** when your intention matches this example:

> Use these selected sources to propose a workflow for a research report.
...
1. **Propose the project workflow.** Propose Git or content-only project workflows from a complete registered batch and current lane contracts without changing Plan or sources. This stage is read-only.
...
A proposal neither changes the Plan nor grants permission to execute.
...
- Public skill: `configure-project-workflow`
- Representative actions: `project_workflow_configure`, `plan_read`
