...
A primary route and declared alternatives with measured readiness requirements.

## Use it when

Use **Inspect and select tools** when your intention matches this example:

> Check which tools can perform this task on my current machine.
...
1. **Inspect declared tool routes.** Read shared tool declarations and executable operation routes; installation and execution remain separately qualified. This stage is read-only.
...
3. **Read measured readiness.** Read measured tools/providers, worker state and selected-project job health. This stage is read-only.
...
An unavailable tool stays unavailable. Uncertain effects are not silently retried with another tool.
...
- Public skill: `select-project-tools`
- Representative actions: `toolchain_catalog`, `toolchain_resolve`, `runtime_status`
