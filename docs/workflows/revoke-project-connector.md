...
The exact grant is revoked and its historical receipts remain inspectable.

## Use it when

Use **Withdraw a connection** when your intention matches this example:

> Revoke this project connector and retain its history.
...
2. **Withdraw the selected grant.** Revoke the exact reviewed project extension version while preserving its history. This stage changes recorded project state.
...
Revocation does not uninstall the provider, remove project data or revoke unrelated grants.
...
- Public skill: `revoke-project-connector`
- Representative actions: `connector_read`, `connector_revoke`, `connector_read`
- Some representative stages can change project records: `connector_revoke`.
