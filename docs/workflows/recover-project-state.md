...
Back up separate project lanes coherently, restore selected Git history to a fresh folder, or recover after client loss. Use for explicit recovery; accepted-version rollback is removed.

## What this helps you do

A checked backup of separate records or a fresh recovery destination.

## Use it when

Use **Back up or recover a project** when your intention matches this example:

> Create and verify a coherent project backup before recovery.
...
1. **Inspect recovery state.** Inspect registered files, unregistered leftovers and database recovery status without repair. This stage is read-only.
2. **Create a coherent backup.** Create and verify a consistent project database and registered-file backup. This stage changes recorded project state.
3. **Verify the selected backup.** Hash-check an exact project backup without restoring it. This stage is read-only.
...
Offline recovery requires a stopped engine. It preserves the original root and never claims failed work succeeded.
...
- Public skill: `recover-project-state`
- Representative actions: `project_recovery_inspect`, `project_backup`, `project_backup_verify`
- Some representative stages can change project records: `project_backup`.
