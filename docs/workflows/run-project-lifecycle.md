...
The owning workflows coordinate entry, planning, work, evidence and closure.

## Use it when

Use **Coordinate end-to-end work** when your intention matches this example:

> Take this project from selected sources through a Plan to a verified result.
...
1. **Open the project session.** Boot one project session with verified Flash and attributed capture. This stage changes recorded project state.
2. **Read the current Plan.** Read a bounded page of the authoritative project Plan. This stage is read-only.
3. **Start the eligible task.** Run the operation already bound to one exact Plan task through its owning bounded task executor and verifier. This stage changes recorded project state.
4. **Inspect output and checks.** Read a recorded run and reconcile its completion evidence; optionally return result and verification bytes. This stage is read-only.
5. **Close at a safe boundary.** Close the exact current session at a safe boundary and detach its capture binding. This stage changes recorded project state.
...
Each step retains its own checks and permissions. A lifecycle request cannot bypass a failed gate.
...
- [Close a project session](close-project-session.md)
- [Back up or recover a project](recover-project-state.md)

<details>
<summary>SDK and MCP reference</summary>

- Public skill: `run-project-lifecycle`
- Representative actions: `session_boot`, `plan_read`, `delta_enter_planned`, `delta_status`, `session_exit`
- Some representative stages can change project records: `session_boot`, `delta_enter_planned`, `session_exit`.
